...
// Meant to be used inside NextUIProvider
...
export default function Canvas({ stlUrl: stlUrl, color } :CanvasProps) {
    ...
    scene.background = null
      ...
      const loadStl = () => {
        console.log(stlUrl)
        if (stlUrl === '') return;
        fetch(stlUrl).then(res => res.blob()).then(res => {
  ...
  return (
		<div>
            <canvas className="rounded w-full h-full"></canvas>
        </div>
  );
}
